package org.lemon.commons.core.utils;
...
public class ServletUtils extends JakartaServletUtil {
    ...
    /**
     * 将字符串渲染到客户端（以 JSON 格式返回，HTTP 状态码默认 200）
     *
     * @param response 渲染对象
     * @param string   待渲染的字符串
     */
    public static void renderString(HttpServletResponse response, String string) {
        renderString(response, HttpStatus.UNAUTHORIZED.value(), string);
    }

    /**
     * 将字符串渲染到客户端（以 JSON 格式返回，支持自定义 HTTP 状态码）
     * <p>
     * 适用于 Spring Security 过滤器链中的各类 Handler，在该层无法使用 Spring MVC 的
     * 高级抽象（如 ResponseEntity），需直接操作 HttpServletResponse。
     *
     * @param response 渲染对象
     * @param status   HTTP 状态码（如 200、401、403）
     * @param string   待渲染的字符串
     */
    ...
    public static boolean isAjaxRequest(HttpServletRequest request) {
        ...
        if (Strings.CS.equalsAny(uri, ".json", ".xml")) {
            return true;
        }
