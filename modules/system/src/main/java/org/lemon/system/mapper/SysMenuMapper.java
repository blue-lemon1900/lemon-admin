...
public interface SysMenuMapper extends BaseMapperPlus<SysMenu, SysMenuVo> {
    ...
    /**
     * 构建多角色对应的菜单ID SQL 子查询
     *
     * @param roleIds 角色ID集合
     * @return 查询菜单ID的 SQL 子查询字符串
     */
    default String buildMenuByRolesSql(Collection<Long> roleIds) {
        String ids = roleIds.stream().map(String::valueOf).collect(Collectors.joining(","));
        return """
                    select srm.menu_id from sys_role_menu srm
                        left join sys_role sr on sr.role_id = srm.role_id
                        where srm.role_id in (%s) and sr.status = '0'
                """.formatted(ids);
    }
     ...
     * @return 权限列表
     */
    default Set<String> selectMenuPermsByRoleIds(Collection<Long> roleIds) {
        ...
        List<String> list = this.selectObjs(
                new LambdaQueryWrapper<SysMenu>()
                        .select(SysMenu::getPerms)
                        .inSql(SysMenu::getMenuId, this.buildMenuByRolesSql(roleIds))
                        .isNotNull(SysMenu::getPerms)
        );
        return new HashSet<>(StreamUtils.filter(list, StringUtils::isNotBlank));
    }
