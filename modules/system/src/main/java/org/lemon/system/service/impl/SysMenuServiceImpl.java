...
import org.lemon.system.domain.entity.SysTenantPackage;
import org.lemon.system.domain.vo.MetaVo;
import org.lemon.system.domain.vo.RouterVo;
import org.lemon.system.domain.vo.SysMenuVo;
import org.lemon.system.mapper.SysMenuMapper;
import org.lemon.system.mapper.SysRoleMapper;
import org.lemon.system.mapper.SysRoleMenuMapper;
import org.lemon.system.mapper.SysTenantPackageMapper;
    ...
    public boolean checkRouteConfigUnique(SysMenuBo menuBo) {
        ...
        for (SysMenu sysMenu : sysMenuList) {
            if (!sysMenu.getMenuId().equals(menuId)) {
                ...
                if (Strings.CI.equalsAny(path, dbPath) && parentId.equals(dbParentId)) {
                    log.warn("[同级路由冲突] 同级下已存在相同路由路径 '{}'，冲突菜单：{}", dbPath, sysMenu.getMenuName());
                    return false;
                } else if (Strings.CI.equalsAny(path, dbPath)
                    ...
                    return false;
                } else if (Strings.CI.equalsAny(routeName, dbRouteName)
