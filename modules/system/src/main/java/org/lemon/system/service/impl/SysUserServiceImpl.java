...
import org.lemon.commons.core.domain.dto.UserDTO;
...
import org.lemon.system.domain.vo.SysUserExportVo;
import org.lemon.system.domain.vo.SysUserVo;
import org.lemon.system.mapper.*;
import org.lemon.system.service.ISysUserService;
...
@Service
public class SysUserServiceImpl implements ISysUserService, UserService, UsernameService {
     ...
     */
    @Override
    public LoginUserInfo loadUserByUsername(String tenantId, String username) {
        SysUser user = baseMapper.loadUserByTenantIdAndUsername(tenantId, username);
        if (ObjectUtil.isNull(user)) {
            log.info("登录用户：{} 不存在.", username);
            throw new UserException(username + "用户不存在");
        } else if (SystemConstants.DISABLE.equals(user.getStatus())) {
            log.info("登录用户：{} 已被停用.", username);
            throw new UserException(username + "用户已被停用");
        }
        ...
        loginUserInfo.setTenantId(user.getTenantId());

        return loginUserInfo;
    }

    @Override
    public LoginUserInfo loadUserByUsername(String username) {
        SysUser user = baseMapper.loadUserByUsername(username);
        if (ObjectUtil.isNull(user)) {
            log.info("登录用户：{} 不存在.", username);
            throw new UserException(username + "用户不存在");
        } else if (SystemConstants.DISABLE.equals(user.getStatus())) {
    ...
    public void saveUserInfo(LoginUserInfo loginUserInfo) {
        // 根据用户ID查询对应的角色信息
        List<SysRoleVo> roleList = roleMapper.selectRolesByUserId(loginUserInfo.getUserId());
        // 设置角色信息
        Set<RoleModel> roles = roleList.stream()
                .map(role -> new RoleModel(role.getRoleId(), role.getRoleName(), role.getRoleKey(), role.getDataScope()))
    ...
    /**
     * 使用 refresh token 换取一组新的令牌
     * <p>
     * 校验 refresh token 有效性，删除旧的 access token 和 refresh token，
     * 重新生成并返回包含新令牌的登录用户信息
     * </p>
     *
     * @param refreshToken 刷新令牌
     * @return 刷新后的登录用户信息（含新的 accessToken 和 refreshToken）
     * @throws ServiceException refresh token 无效或已过期时抛出
     */
    @Override
    public LoginUserInfo updateToken(String refreshToken) {
        LoginUserInfo loginUserInfo = RedisUtils.getCacheObject(REFRESH_TOKEN + refreshToken);
     ...
     */
    private void generateAccessToken(LoginUserInfo loginUserInfo) {
        // 设置访问token
        String uuid = IdUtil.fastSimpleUUID();
        loginUserInfo.setAccessToken(uuid);

        // 生成刷新令牌
        String refreshUuid = IdUtil.fastSimpleUUID();
        // 设置刷新token
